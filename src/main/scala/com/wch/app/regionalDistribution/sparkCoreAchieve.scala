...
object sparkCoreAchieve {
  def main(args: Array[String]): Unit = {
    ...
    // 模板代码
    val conf: SparkConf = new SparkConf().setAppName(s"${this.getClass.getSimpleName}").setMaster("local[1]")
    ...
    val res = groupedRDD.map(x => {
      ...
      // 竞价成功率
      val biddingSuccRate: Double = if (joinBidding == 0) 0 else succBidding.toDouble / joinBidding.toDouble
      ...
      // 点击率
      val clickRate: Double = if (showCount == 0) 0 else clickCount.toDouble / showCount.toDouble
      // DSP
    ...
    // res.foreach(println(_))  // 循环打印
    // res.saveAsTextFile(outputPath)  // 存储到磁盘中

    // 创建SQL语句
    val sql = "INSERT INTO area_distribution VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)"
    // 遍历RDD 进行插入数据库操作
    res.foreach(x => {
      val params = Array(x._1, x._2, x._3, x._4, x._5, x._6, x._7, x._8, x._9, x._10, x._11, x._12, x._13)
