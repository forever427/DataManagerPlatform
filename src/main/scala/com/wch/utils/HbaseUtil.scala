...
object HbaseUtil {
  ...
  // 向HBASE表中批量插入数据
  def insertTableBatch(tableName: String, data: RDD[(String, Map[String, Int])]) = {
  ...
  def main(args: Array[String]): Unit = {
    //    createTable("test:test1", Array("i"))
    //    insertTable("test:test1", "1", "i", "age", "22")
    //    scanDataFromHTable("test:test1","i", "age")
    deleteRecord("test:test1", "1", "i", "age")
    close()
  }
